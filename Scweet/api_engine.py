...
class ApiEngine:
    def __init__(
        ...
        configured_mode_raw = _cfg(config, "api_http_mode", HTTP_MODE_AUTO)
    ...
    async def get_profile_tweets(self, request, on_tweets_page: Optional[Any] = None):
        ...
        if max_empty_pages is None:
            max_empty_pages = self._coerce_positive_int(_cfg(self.config, "max_empty_pages", 1)) or 1
        ...
        configured_switches_raw = _cfg(self.config, "max_account_switches", 2)
        try:
    ...
    def _resolve_page_size(self, *, runtime_hints: Optional[dict[str, Optional[int]]]) -> int:
        configured_page_size = self._coerce_positive_int(_cfg(self.config, "api_page_size", 20)) or 20
