...
- A large search over a wide time range now returns far more of the data. A cursor chain of X stops at a depth limit while tweets remain in the range, and the run used to end there and report success. The engine now detects this (a full last page with no cursor) and continues the search from the time of the oldest tweet it saw, re-querying only the older, unfetched part of the range down to `scheduler_min_interval_s`. A day of tweets that returned one page now returns the whole day. New config: `max_interval_depth` (6) caps the continuation; set it to `0` to turn it off.
